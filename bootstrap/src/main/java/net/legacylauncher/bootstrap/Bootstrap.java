    ...
    private Task<Void> defTask() {
        return new Task<Void>("defTask") {
            ...
            protected Void execute() throws Exception {
                ...
                if (updateMetaFile != null) {
                ...
                } else if (!getIgnoreUpdate() && !getIgnoreSelfUpdate()) {
                    ...
                    log.info("Updates disabled, skipping remote update metadata fetch");
                    updateMeta = null;
                }
    ...
    private Task<LocalLauncherTask> getLocalLauncher(final RemoteLauncher remote) {
        return new Task<LocalLauncherTask>("getLocalLauncher") {
            ...
            protected LocalLauncherTask execute() throws Exception {
                ...
                LocalLauncher local = null;
                try {
                    local = new LocalLauncher(getTargetJar(), getTargetLibFolder());
                } catch (LauncherNotFoundException lnfE) {
                    log.warn("Could not find launcher at configured path: {} / {}", getTargetJar(), getTargetLibFolder());
                }

                if (local == null) {
                    Path[] candidateJars = new Path[] {
                            bootstrapJar != null && bootstrapJar.getParent() != null ? bootstrapJar.getParent().resolve("launcher.jar") : null,
                            bootstrapJar != null && bootstrapJar.getParent() != null ? bootstrapJar.getParent().resolve("launcher").resolve("launcher.jar") : null,
                            Paths.get("launcher", "launcher.jar").toAbsolutePath(),
                            Paths.get("launcher.jar").toAbsolutePath()
                    };
                    Path[] candidateLibs = new Path[] {
                            bootstrapJar != null && bootstrapJar.getParent() != null ? bootstrapJar.getParent().resolve("libraries") : null,
                            bootstrapJar != null && bootstrapJar.getParent() != null ? bootstrapJar.getParent().resolve("launcher").resolve("libraries") : null,
                            Paths.get("launcher", "libraries").toAbsolutePath(),
                            Paths.get("libraries").toAbsolutePath()
                    };

                    for (int i = 0; i < candidateJars.length; i++) {
                        Path cJar = candidateJars[i];
                        Path cLib = candidateLibs[i];
    ...
    private static class RunningConditionsResult {
        ...
        public String formatMessage() {
            ...
            if (brokenPath != null) {
                ...
                appendLine(message, "Не запускайте Java-приложения в директориях, чей путь содержит «!». Переместите WlLauncher в другую папку.");
            }
